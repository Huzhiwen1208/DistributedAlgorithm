...
func (p *BankC) RemittanceIn(amount int64) {
	p.Balance += amount
	// 通知分布式事务管理器
	p.Notify("RemittanceIn")
}

func (p *BankC) RemittanceOut(amount int64) error {
	if p.Balance < amount {
		return xerrors.Errorf("BankC 余额不足")
	}
...
func (p *BankC) Notify(act string) error {
	// connect 分布式事务管理器
...
func (p *BankC) Try(amount int64) error {
	if amount > 0 {
	...
	if -amount > p.Balance {
		p.Notify("Try failed")
		return xerrors.Errorf("BankC{Try} 余额不足")
	}
...
func (p *BankC) Confirm(amount int64) error {
	if amount > 0 {
		p.Balance += amount
	...
	if -amount > p.Balance {
		...
		return xerrors.Errorf("BankC{Confirm} 余额不足")
	}
	p.Balance += amount
